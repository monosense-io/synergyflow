...
**OPA Integration** (Spring Security):
```java
@Component
    ...
    spec:
      ...
      - name: frontend
        ...
        - name: NEXT_PUBLIC_API_URL
          value: https://app.synergyflow.io/api
```
...
**MyBatis-Plus Plugins Configuration**:

MyBatis-Plus 3.5.9+ requires separate plugin dependencies and configuration. The following plugins are essential for SynergyFlow:
...
public class MyBatisPlusConfig {
    ...
    public MybatisPlusInterceptor mybatisPlusInterceptor() {
        ...
        // 1. Pagination (requires mybatis-plus-jsqlparser)
        PaginationInnerInterceptor pagination = new PaginationInnerInterceptor(DbType.POSTGRE_SQL);
        pagination.setMaxLimit(500L);  // Max page size limit
        interceptor.addInnerInterceptor(pagination);

        // 2. Optimistic Locking (version field @Version annotation)
        interceptor.addInnerInterceptor(new OptimisticLockerInnerInterceptor());

        // 3. Multi-Tenant (tenant_id column injection)
        ...
        // 4. Block Attack (prevent full table update/delete)
        interceptor.addInnerInterceptor(new BlockAttackInnerInterceptor());

        // 5. Illegal SQL (prevent dangerous SQL patterns)
